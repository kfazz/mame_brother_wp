...
void upd765_family_device::seek_continue(floppy_info &fi)
{
	for(;;) {
		...
		case SEEK_WAIT_STEP_SIGNAL_TIME_DONE:
			...
			if (slow)
			delay_cycles(fi,500 *(32-((spec >> 12) <<1)));
			else
...
void upd765_family_device::read_data_continue(floppy_info &fi)
{
	for(;;) {
		...
		case HEAD_LOAD:
			LOGSTATE("HEAD_LOAD\n");
			if (slow)
			delay_cycles(fi, 500*((spec & 0x00fe) << 1));
			else
...
uint8_t hd63266_device::get_st3(floppy_info &fi)
{
	uint8_t st3 = command[1] & 7;
	if(fi.ready)
		st3 |= ST3_RY;
...
void hd63266_device::hd_w(uint8_t data)
{
	if (drq) {
		return dma_w(data);
	}
...
int hd63266_device::check_command()
{
	...
	case 0xb:
	// 0..01011 specify 2
	// LOGCOMMAND("unhandled command %x\n", command[0] & 0x1f); return C_INVALID;
